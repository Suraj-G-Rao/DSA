...
map< data_type_1 , data_type_2 > variable;
map< data_type_1 , data_type_2 , greater<data_type_1> > variable;  --> TO STORE THE KEYS IN DESCENDING ORDER
...
m.find(ELE)  --> RETURNS AN ITERATOR POINTING TO FIRST OCCURENCE OF THAT ELEMENT IF EXISTS ELSE RETURNS END ITREATOR --> O(log(N))
m.count(ELE) --> RETURNS NO. OF TIMES THAT ELE IS REPEATED  --> O(K+log(N)) WHERE K IS THE NO. OF OCC
...
int main(){
    ...
    map<int,int, greater<int>>m; // --> TO STORE THE KEYS IN DESCENDING ORDER
    cout<<m[0];
    // INSERTION : O(log(N))
