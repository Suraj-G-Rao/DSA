...
int main(){
    ...
    unordered_multimap<int,int>m={{1,1},{2,2},{3,3}};
    // unordered_multimap<int,int, greater<int>>m; // --> TO STORE THE KEYS IN DESCENDING ORDER

    // INSERTION : O(1) ON AVG CASE O(N) ON WORST CASE

    m.insert({5,5});
    m.insert({5,6});
    // m[5]=6; // THIS OPERATION IS NOT VALID IN UNORDERED MULTIMAP  
    ...
    //unorderd_multimap<int,int> :: iterator it;
    ...
    // unordered_multimap<int,int> :: reverse_iterator it;
